...
@extends('waiter.sidebar',['nav_title'=>"Menu"])
...
<div class="flex items-center gap-2 text-sm mb-6">
    <a href="{{route('OrderMenu')}}" class="cursor-pointer">
        <span class="text-gray-500 hover:text-primary transition-all duration-200">Orders Menu</span>
    </a>
    <i data-lucide="chevron-right" class="w-4 h-4 text-gray-400"></i>
    <a href="{{route('getMenu', ['table_id' => $tableId] )}}" class="cursor-pointer">
...
<div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6 md:mb-8">
    <div>
        ...
        <p class="text-gray-500 text-sm md:text-base">View the menu, take orders, and serve customers faster with ease.</p>
    </div>
            ...
            <div class="space-y-4">
                <div class="flex gap-4 pb-4">
                    <div class="flex-1 min-w-0">
                        ...
                        @endif
                        <h4 class="mt-2 menu-title item_name" data-tableid="{{ $tableId }}" data-category="{{ $item->category_id }}" data-id="{{ $item->id }}">{{ $item->item_name}}</h4>
                        @if($seating_type == 1)
                        <h5 class="item_price price" data-price="{{ $item->ac_price}}">₹{{ $item->ac_price}}</h5>
                        @else
...
<script>
    $(document).on('click', '.item_details', function() {
        ...
        $.ajax({
            url: "/ordermenu/cart_view/" + itemId,
            type: "GET",
    ...
    $(document).on('click', '#addToCartBtn', function() {
        ...
        if (action == 'update_cart') {
            ...
            $.ajax({
                url: '/ordermenu/cart', // Replace with your actual route
                type: 'POST',
                data: {
            ...
            $.ajax({
                url: '/ordermenu/add-to-cart', // Replace with your actual route
                type: 'POST',
                data: {
                    _token: "{{ csrf_token() }}", // CSRF token
                    item_id: itemId,

        ...
        <div class="p-4">
            <div class="space-y-4">
                ...
                <div class="flex gap-4 pb-4 mt-3 md:justify-between">
                    ...
                    <a href="{{ route('RemoveOrderfromCart', ['item_id' => $details->id]) }}" style="color:#fff" class="float-right cart_remove md:text-2xl" onclick="return confirm(' you want to delete?');" id="" data-id="">&times;</a>
                </div>
                @endforeach
                @else
    ...
    $(document).ready(function() {
        $('.quantity input[type="number"]').on('input', function() {
            ...
            if (newVal >= min && newVal <= max) {
                $.ajax({
                    url: "{{ route('UpdateTakeAwayQuantity') }}",
                    method: "PATCH",
                    data: {
                        _token: '{{ csrf_token() }}',
                        id: itemId,
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                if (res.success) {
                    ...
                    $.ajax({
                        url: "/take-away-order/printorder/" + res.order_id,
                        type: "GET",
                        success: function(printRes) {

                            if (printRes.success) {
                                alert("Order placed successfully!");
                                window.location.href = "{{ route('admin_order') }}";
                            }
                        }
                    });

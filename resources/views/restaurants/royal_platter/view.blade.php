...
<div class="view_image">
    ...
    <a href="{{ route('royalplatter.index', session('table_name')) }}">
...
<div class="container">
    <div class="content-wrapper" id="main_section">
        ...
        <div class="row justify-content-between mt-5">
                ...
                <div class="text">
                    ...
                    <div class="btn-group mt-2">
                        ...
                        <a type="button" href="{{ route('add_to_cart', $row->id) }}" class="btn btn-danger add_to_cart btn-sm" name="add_to_cart" id="{{$item->id}}">
                            Add</a>
                        @endif -->
                        <a type="button" href="{{ route('royalplatter.add_to_cart', $row->id) }}" class="btn btn-danger add_to_cart btn-sm" name="add_to_cart" id="{{$item->id}}">
                            Add</a>
                    </div>
                </div>
            </div>
            <div class="col-4">
                <div class="image">
                    <a href="{{ route('royalplatter.view', $row->id) }}"><img class="product_image float-right" src="{{ url('items/' . $row->picture) }}"></a>
                </div>
            </div>
        </div>
        @endif
        ...
        <div class="row">
            <div class="col-md-12 mt-3 mb-2">
                <!-- @if (empty(session()->has('session_code')))
                <a type="button" href="javascript:void(0);" onclick="formToggle();" class="btn btn-success add_to_cart col-12" name="add_to_cart" id="{{$item->id}}">
                    <i class="fas fa-plus"></i> Add to cart</a>
                @else
                <a type="button" href="{{ route('add_to_cart', $item->id) }}" class="btn btn-success add_to_cart col-12" name="add_to_cart" id="{{$item->id}}">
                    <i class="fas fa-plus"></i> Add to cart</a>
                @endif -->

                <a type="button" href="{{ route('royalplatter.add_to_cart', $item->id) }}" class="btn btn-success add_to_cart col-12" name="add_to_cart" id="{{$item->id}}">
                    <i class="fas fa-plus"></i> Add to cart</a>
            </div>

                        ...
                        <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="content_{{ $category->id }}">
                            <ul class="list-unstyled category">
                                ...
                                <div class="row justify-content-between mt-4">
                                    <div class="col-4">
                                        <div class="text">
                                            ...
                                            <div class="btn-group mt-1">
                                                ...
                                                <a type="button" href="{{ route('scantable.add_to_cart', $item->id) }}" class="btn btn-danger add_to_cart btn-sm" name="add_to_cart" id="{{$item->id}}">
                                                    Add</a>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-4">
                                        <div class="image">
                                            <a href="{{ route('scantable.view', $item->id) }}"><img class="product_image float-right" src="{{ url('items/' . $item->picture) }}"></a>
                                        </div>
                                    </div>
                                </div>
                                @endif
...
<script>
    ...
    window.onbeforeunload = function() {
        // $.ajax({
        //     url: "{{ route('scantable.tab_close',session('table_id')) }}",
        //     method: "delete",
